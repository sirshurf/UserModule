...
class User_IndexController extends Zend_Controller_Action
{
    ...
    public function indexAction ()
    {
        ...
        $grid = new Ingot_JQuery_JqGrid('users', 
        new Ingot_JQuery_JqGrid_Adapter_DbTableSelect($objSelect));
        $grid->setIdCol(User_Model_Db_Users::COL_ID_USERS);
        $strUrl = $this->view->url(
        array('module' => 'user', 'controller' => 'index', 'action' => 'view'), 
        null, true, false);
        $grid->setOption('ondblClickRow', 
        "function(rowId, iRow, iCol, e){ if(rowId){  document.location.href ='" .
         $strUrl . "/UserId/'+rowId } }");
        $grid->addColumn(
        new Ingot_JQuery_JqGrid_Column(User_Model_Db_Users::COL_LOGIN));
        $grid->addColumn(
        new Ingot_JQuery_JqGrid_Column(User_Model_Db_Users::COL_FIRST_NAME));
        $grid->addColumn(
        new Ingot_JQuery_JqGrid_Column(User_Model_Db_Users::COL_LAST_NAME));
        $grid->addColumn(
        new Ingot_JQuery_JqGrid_Column_Decorator_Link(
        new Ingot_JQuery_JqGrid_Column(User_Model_Db_Users::COL_EMAIL), 
        array('link' => 'mailto:%s')));
        $grid->registerPlugin(new Ingot_JQuery_JqGrid_Plugin_ToolbarFilter());
        $this->view->grid = $grid->render();
        $arrActions = array();
        $arrActions[] = array('module' => 'user', 'controller' => 'index', 
        "action" => "edit", "name" => 'Add New User');
    ...
    public function editAction ()
    {
        ...
        if (! empty($intUserID)) {
            ...
            if (! empty($objUserRowSet)) {
                ...
                $strUrl = $this->view->url(
                array('module' => 'users', 'controller' => 'index', 
            ...
            if ($objForm->isValid($formData)) {
                ...
                if ($intUserID) {
                    ...
                    foreach ((array) $arrExtraData as $strExtraDataClass) {
                        ...
                        $objExtraData->setMainRow($objUserRow);
                        if ($objExtraData instanceof User_Model_User_Extra_Iterface) {
                            $objForm->save($objForm->getSubForm('subformUserDetails')->getValues());
                        }
                    }
                    ...
                    //}
                    $strUrl = $this->view->url(
                    array('module' => 'user', 'controller' => 'index', 
                    'action' => 'view', "UserId" => $intUserID), null, true);
                    $this->_redirect($strUrl);
                } else {
        ...
        $arrButtons[] = array('module' => 'user', 'controller' => 'index', 
        "action" => "edit", 
        "onClick" => '$("#' . $objForm->getAttrib('id') . '").submit();', 
        "name" => 'LBL_BUTTON_USER_EDIT_SAVE');
        if (! empty($intUserID)) {
            $arrButtons[] = array('module' => 'user', 'controller' => 'users', 
            "action" => "view", "name" => 'LBL_BUTTON_USER_DETAILS', 
            "params" => array("UserId" => $intUserID));
        }
        $arrButtons[] = array('module' => 'user', 'controller' => 'index', 
        "action" => "index", "name" => 'LBL_BUTTON_USER_LIST');
    ...
    public function viewAction ()
    {
        ...
        $objUserDataSelect->where(User_Model_Db_Users::COL_ID_USERS . " = ?", 
        $intUserId);
        ...
        // Create form
        $arrButtons[] = array('module' => 'user', 
        'controller' => 'index', "action" => "edit", 
        "name" => "LBL_BUTTON_USER_DETAILS_EDIT", 
        "params" => array("UserId" => $intUserId));
        $arrButtons[] = array('module' => 'user', 
        'controller' => 'authentication', "action" => "takepermition", 
        "name" => "LBL_BUTTON_USER_ASSUME_USER_PERMITION", 
        "params" => array("UserId" => $intUserId));
        $arrButtons[] = array('module' => 'user', 'controller' => 'index', 
        "action" => "index", "name" => "LBL_BUTTON_USER_LIST");
        $this->view->arrActions = $arrButtons;
    ...
    public function inituserdetailsAction ()
    {
        ...
        $strUrl = $this->view->url(
        array('module' => 'semesters', 'controller' => 'semester', 
        'action' => 'index'), null, true);
        $this->_redirect($strUrl);
    }
